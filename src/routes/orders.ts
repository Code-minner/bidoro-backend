...
router.get("/", authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // DB allowed: pending, confirmed, processing, shipped, delivered, completed, cancelled, disputed
    if (status) {
        ...
        data: {
          ...
          buyer: buyerProfile
            ? {
                ...
                firstName: buyerProfile.name?.split(" ")[0] || "",
                lastName:
                  buyerProfile.name?.split(" ").slice(1).join(" ") || "",
                email: buyerProfile.email,
                phone: buyerProfile.phone,
                avatar: buyerProfile.profile_picture,
              }
          ...
          seller: sellerProfile
            ? {
                ...
                firstName: sellerProfile.name?.split(" ")[0] || "",
                lastName:
                  sellerProfile.name?.split(" ").slice(1).join(" ") || "",
                email: sellerProfile.email,
                avatar: sellerProfile.profile_picture,
              }
            : null,
  ...
  try {
    ...
    if (!sellerId || !items || !items.length || !shippingAddress) {
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: sellerId, items, and shippingAddress",
      });
    }
    ...
    for (const item of items) {
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            "Each item must have productId, productName, quantity, and unitPrice",
        });
      }
    }
      ...
      switch (action) {
        case "updateStatus": {
          ...
          if (
            ...
            !VALID_TRANSITIONS[currentStatus].includes(dbStatus)
          ) {
            return res.status(400).json({
              success: false,
