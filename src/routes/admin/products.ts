...
router.get('/', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    const transformedProducts = (products || []).map(product => ({
      ...
      primaryImage: product.product_images?.find((img: any) => img.is_primary)?.image_url || 
...
router.get('/stats', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    (allProducts || []).forEach(p => {
      ...
      const vStatus = (p.verification_status || 'pending') as keyof typeof verificationCounts;
 ...
 */
router.get('/pending', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
...
router.post('/:productId/verify-receipt', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    res.status(501).json({
      success: false,
      error: 'Receipt verification not yet implemented. Please review manually.'
    });
...
router.delete('/:productId', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    try {
      await supabase.from('product_verifications').delete().eq('product_id', productId);
    } catch {}
