...
router.post('/become-seller', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!seller_type || !validSellerTypes.includes(seller_type)) {
      return res.status(400).json({
        success: false,
        message: 'Invalid seller type. Must be one of: products, services, both'
      });
    }
...
router.put('/seller-type', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const { seller_type } = req.body;
    ...
    data: {
      seller_types: [
        { 
          value: 'products', 
          label: 'Sell Products', 
          description: 'Sell physical items like electronics, fashion, gadgets, etc.',
          features: ['Inventory management', 'Shipping options', 'Product photos']
        },
        { 
          value: 'services', 
          label: 'Offer Services', 
          description: 'Provide services like repairs, tutoring, consulting, etc.',
        ...
        { 
          ...
          features: ['Full marketplace access', 'Flexible offerings', 'Multiple revenue streams']
        }
      ]
    }
  });
...
router.put('/switch-role', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const { role } = req.body;
...
// Send verification email
router.post('/send-verification', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
