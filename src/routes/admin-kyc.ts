...
router.get("/applications", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const summary = {
      ...
      under_review:
        stats?.filter((s) => s.status === "under_review").length || 0,
...
router.post(
  "/applications/:applicationId/document-feedback",
  async (req: AuthRequest, res: Response) => {
    try {
      const { applicationId } = req.params;
      ...
      avg_processing_time:
        applications
          ...
          .reduce((acc, a) => {
            ...
            const days =
              (approved.getTime() - submitted.getTime()) /
              (1000 * 60 * 60 * 24);
            return acc + days;
          }, 0) /
          (applications?.filter((a) => a.approved_at && a.submitted_at)
            .length || 1) || 0,
    };
